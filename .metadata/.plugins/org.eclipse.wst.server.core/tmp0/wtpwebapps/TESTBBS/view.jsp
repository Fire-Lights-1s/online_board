...
<body>
	...
	<script type="text/javascript">
		//수정 버튼을 클릭시 클릭한 댓글에 수정을 위한 태그추가
		var updateID=0;
		function update(replyID, bbsID){
			if (document.getElementById('reComment') != null) {
				cancel("reComment");
			}
			if (document.getElementById('reCommentUpdate') != null) {
				cancel("reCommentUpdate");
				var reContent = document.getElementById('content' + this.updateID);
				reContent.setAttribute("style", "");
			}
			this.updateID = replyID;
			var Content = document.getElementById('update' + replyID).innerText;
			var reComment = document.getElementById('update' + replyID);
			var replyContent = document.getElementById('content' + replyID);
			replyContent.setAttribute("style", "display:none");
			var reCommentUpdate = document.createElement("form")
			reCommentUpdate.setAttribute("id", "reCommentUpdate");
			reCommentUpdate.setAttribute("method", "post");
			reCommentUpdate.setAttribute("action", "replyUpdateAction.jsp?bbsID="+ bbsID);
			
			reCommentUpdate.innerHTML = "<textarea  class='form-control' placeholder='답글 내용'"
					+ "name='replyContent' maxlength='1000' style='height: 70px; resize: none;'>" + Content + "</textarea>"
					+ "<input type='hidden' name='replyID' value='" + replyID + "'>" 
					+ "<input type='submit' class='btn btn-primary pull-right' value='답글 수정'>"
					+ "<a class='btn btn-primary pull-right' href='javascript:void(0);'" 
					+ "name='reCommentUpdate' onclick='updateCancel(this.name, "+ replyID +");' >취소</a>";
			
			reComment.appendChild(reCommentUpdate);
		}
		function cancel(areaID) {
			var reComment = document.getElementById(areaID);
			var parent = reComment.parentElement;
			parent.removeChild(reComment);

		}
		function updateCancel(areaID,replyID) {
			var reComment = document.getElementById(areaID);
			var parent = reComment.parentElement;
			parent.removeChild(reComment);
			var reContent = document.getElementById('content' + replyID);
			reContent.setAttribute("style", "");
		...
		function reComment(replyID, groupID, bbsID, groupUserID) {
			...
			if (document.getElementById('reCommentUpdate') != null) {
				...
				reCommentForm.innerHTML = "<textarea class='form-control' placeholder='답글 내용'"
				...
				<tbody id="reply<%=replyID%>">
					<tr>
						<td style="text-align: left;" colspan="2" >
							<%=replyList.get(i).getReplyUserName()
								.replaceAll("<", "&lt")
								.replaceAll(">", "&gt")
								.replaceAll("\n", "<br>")%>(<%=replyUserID%>)<br>
							<span id='update<%=replyID%>'><span id ='content<%=replyID%>'><%=replyList.get(i).getReplyContent()
									.replaceAll("<", "&lt")
									.replaceAll(">", "&gt")
									.replaceAll("\n", "<br>")%></span></span><br> 
							...
							<%
 							...
 							<a href="javascript:void(0);"
								onclick="update(<%=replyID%>, <%=bbsID%>);">수정</a> 
 							<a href="replyDeleteAction.jsp?bbsID=<%=bbsID%>&replyID=<%=replyID%>"
				...
				<tbody id="reply<%=reComentID%>">
					<tr>
						...
						<td style="text-align: left;">
							<%=reComent.get(j).getReplyUserName()
								.replaceAll("<", "&lt")
								.replaceAll(">", "&gt")
								.replaceAll("\n", "<br>")%>(<%=replyUserID%>)
							<%if( !replyDAO.getGroupUserName(reComent.get(j).getGroupUserID()).equals(null) &&
								...
								▶<%=replyDAO.getGroupUserName(reComent.get(j).getGroupUserID())%>(<%= reComent.get(j).getGroupUserID()%>)
							<%} %><br>
							<span id='update<%=reComentID%>'><span id ='content<%=reComentID%>'><%=reComent.get(j).getReplyContent()
								.replaceAll("<", "&lt")
								.replaceAll(">", "&gt")
								.replaceAll("\n", "<br>")%></span></span><br> 
							...
							<%
 							...
 							%> 
 								<a href="javascript:void(0);"
									onclick="update(<%=reComentID%>, <%=bbsID%>);">수정</a>
								<a href="replyDeleteAction.jsp?bbsID=<%=bbsID%>&replyID=<%=reComentID%>"
									onclick="return confirm('정말로 삭제하시겠습니까?')" >삭제</a>
							<%
							}
							%></td>
